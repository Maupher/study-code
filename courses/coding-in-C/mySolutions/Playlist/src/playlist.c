...
Playlist* Playlist_create(char Playlist_name, char created_by, time_t timestamp);

//Funktion for creating a Song
Song* Song_create(Playlist* p_playlist, char c_name, char c_artist, uint16_t playtime, time_t timestamp);
...
Song* Song_get_by_id(Playlist* p_playlist, char name);
